...
- **Exclusive orphan reservation** (`JobDriver_FinishOrphanedUft`). Orphans come from a shared pool, but `JobDriver_DoBill` only `ReserveAsManyAsPossible`'s its queued target — best-effort, it never fails the job. So two crafters scanning in the same tick can both be dispatched to one orphan; the loser walks over to bind a UFT the winner already finished-and-destroyed. Vanilla's bind toil guards that case (`{ Destroyed: false }`), but CommonSense's `JobDriver_DoBill` reimplementation drops the guard and logs "Tried to bound destroyed UnfinishedThing". Our finish job uses a `JobDriver_DoBill` subclass that reserves the single UFT *exclusively*, so the duplicate dispatch fails cleanly at start and never reaches the bind.
